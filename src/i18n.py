...
_TRANSLATIONS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "input_help_body": "**{language}** uses a non-Latin script your keyboard probably can't type. Turn on a keyboard layout for {language} — **Windows:** `Win`+`Space` · **macOS:** `Ctrl`+`Space` · **mobile:** hold the 🌐 key. No keyboard set up? Use an [online keyboard]({url}) and copy-paste.",
    ...
    "de": {
        ...
        "input_help_body": "**{language}** nutzt eine nicht-lateinische Schrift, die deine Tastatur vermutlich nicht kann. Aktiviere ein {language}-Tastaturlayout — **Windows:** `Win`+`Leertaste` · **macOS:** `Strg`+`Leertaste` · **Handy:** 🌐-Taste gedrückt halten. Keine Tastatur eingerichtet? Nutze eine [Online-Tastatur]({url}) und kopiere den Text.",
    ...
    "es": {
        ...
        "input_help_body": "**{language}** usa una escritura no latina que tu teclado probablemente no puede escribir. Activa una distribución de {language} — **Windows:** `Win`+`Espacio` · **macOS:** `Ctrl`+`Espacio` · **móvil:** mantén pulsada la tecla 🌐. ¿Sin teclado configurado? Usa un [teclado en línea]({url}) y copia y pega.",
    ...
    "uk": {
        ...
        "input_help_body": "**{language}** використовує нелатинську абетку, яку твоя клавіатура, напевно, не вводить. Увімкни розкладку {language} — **Windows:** `Win`+`Пробіл` · **macOS:** `Ctrl`+`Пробіл` · **смартфон:** утримуй клавішу 🌐. Немає розкладки? Скористайся [онлайн-клавіатурою]({url}) і скопіюй текст.",
    ...
    "pl": {
        ...
        "input_help_body": "**{language}** używa pisma niełacińskiego, którego klawiatura raczej nie obsłuży. Włącz układ {language} — **Windows:** `Win`+`Spacja` · **macOS:** `Ctrl`+`Spacja` · **telefon:** przytrzymaj klawisz 🌐. Brak układu? Skorzystaj z [klawiatury online]({url}) i skopiuj tekst.",
    ...
    "he": {
        ...
        "input_help_body": "**{language}** נכתבת בכתב לא-לטיני שהמקלדת שלך כנראה לא תומכת בו. הפעל פריסת מקלדת {language} — **Windows:** `Win`+`רווח` · **macOS:** `Ctrl`+`רווח` · **נייד:** החזק את מקש ה-🌐. אין מקלדת? השתמש ב[מקלדת מקוונת]({url}) והעתק.",
    ...
    "ar": {
        ...
        "input_help_body": "**{language}** تُكتب بخط غير لاتيني قد لا تدعمه لوحة مفاتيحك. فعّل تخطيط لوحة مفاتيح {language} — **Windows:** `Win`+`مسافة` · **macOS:** `Ctrl`+`مسافة` · **الهاتف:** اضغط مطوّلًا على مفتاح 🌐. لا توجد لوحة مفاتيح؟ استخدم [لوحة مفاتيح عبر الإنترنت]({url}) وانسخ النص.",
